...
idioam_lang(){
  source <(curl -sSl 'https://raw.githubusercontent.com/Lordsniffer22/Hackwell101/main/lang/lang')
  ...
  for arch in $listarq; do
    if ! wget -O $lang_dir/${list_lang[$lg]}/$arch "https://raw.githubusercontent.com/Lordsniffer22/Hackwell101/main/lang/${list_lang[$lg]}/$arch" &>/dev/null ;then
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  ...
  check_sistem
	...
	wget -O $udp_file/limitador.sh "https://raw.githubusercontent.com/Lordsniffer22/Hackwell101/main/limitador.sh" &>/dev/null
...
[Unit]
Description=UDPserver Service by @hackwell101
...
vps_info() {
  ...
  print_center -ama ' Version: ⇢ 4.6-Lite'
...
menu_main() {
  ...
  msg -bar3
  echo " $(msg -verd "[1]") $(msg -verm2 '>') $(msg -verd "${a6:-UDP REQUEST♞}")"
  # echo " $(msg -verd "[2]") $(msg -verm2 '>') $(msg -ama "${a8:-UDP Request}")"
  echo " $(msg -verd "[2]") $(msg -verm2 '>') $(msg -teal "${a11:-Boost UDP Speeds⚡ $(msg -verm2 '[!Caution!]')}")"
  echo " $(msg -verd "[3]") $(msg -verm2 '>') $(msg -teal "${a10:-VPS Info🌦️}")"
  echo -e " $(msg -verd "[4]") $(msg -verm2 '>') $(msg -azu "${a4:-INICIAR/DETENER UDPserver}") $estado"
  echo " $(msg -verd "[5]") $(msg -verm2 '>') $(msg -azu "CHANGE LANGUAGE")"
  echo " $(msg -verd "[11]") $(msg -verm2 '>') $(msg -verm2 "${a3:-Uninstall UDP Request}")"
	msg -bar3
  back
...
menu_udp_request(){
  clear
	title "${a1:-HYBRID UDPRequest MANAGER} BY @hackwell101"
	print_center -ama 'Hybrid UDPRequest SCRIPT by Ted Hackwell'
	print_center -ama 'UDP client App ┈➤SocksIP'
	msg -bar
    ...
    msg -bar3
		echo " $(msg -verd "[1]") $(msg -verm2 '>') $(msg -verd "${a6:-CREAR CLIENTE}")"
		echo " $(msg -verd "[2]") $(msg -verm2 '>') $(msg -verm2 "${a7:-REMOVER CLIENTE}")"
		echo " $(msg -verd "[3]") $(msg -verm2 '>') $(msg -ama "${a8:-RENOVAR CLIENTE}")"
		echo " $(msg -verd "[4]") $(msg -verm2 '>') $(msg -azu "${a9:-BLOQUEAR/DESBLOQUEAR CLIENTE}")"
		echo " $(msg -verd "[5]") $(msg -verm2 '>') $(msg -blu "${a10:-DETELLES DE LOS CLIENTES}")"
		echo " $(msg -verd "[6]") $(msg -verm2 '>') $(msg -azu "${a11:-LIMITADO DE CUENTAS}")"
		msg -bar3
